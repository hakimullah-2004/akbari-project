...
export default function NewSalePage() {
  ...
  const addToCart = (product: Product) => {
    ...
    if (existing) {
      setCart(cart.map(c => c.productId === product.id
        ? { ...c, quantity: c.quantity + 1, totalPrice: (c.quantity + 1) * c.unitPrice * (1 - c.discount / 100) }
        : c));
    } else {
      const price = parseFloat(product.salePrice);
      setCart([...cart, {
        productId: product.id, productName: product.name, unit: product.unit,
        quantity: 1, unitPrice: price, discount: 0, totalPrice: price,
      }]);
    }
    setProductSearch("");
  ...
  const handleSubmit = async () => {
    if (cart.length === 0) { toast.error("سبد فروش خالی است"); return; }
    setSaving(true);
    try {
      const res = await fetch("/api/sales", {
        ...
        body: JSON.stringify({
          ...
          discountAmount, paidAmount, notes,
        }),
      });
      const data = await res.json();
      if (res.ok) {
        toast.success(`بل ${data.invoiceNumber} ثبت شد`);
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <ShoppingCart className="w-6 h-6 text-green-600" /> فروش جدید
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-64 overflow-y-auto">
                  {filteredProducts.length === 0 ? (
                    ...
                    >
                      ...
                      <div className="text-green-700 font-semibold text-sm">{formatCurrency(p.salePrice)}</div>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            {cart.length === 0 ? (
              <div className="text-center py-12 text-gray-400">
                ...
                <p className="text-sm">سبد فروش خالی است</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="text-right px-3 py-2 font-semibold text-gray-600">قیمت</th>
                      <th className="text-right px-3 py-2 font-semibold text-gray-600 hidden sm:table-cell">تخفیف%</th>
                      ...
                      <tr key={i}>
                        ...
                        <td className="px-3 py-2 hidden sm:table-cell">
                          <input type="number" min="0" max="100" value={item.discount}
                            ...
                            className="w-16 px-2 py-1 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                        </td>
        ...
        <div className="space-y-4">
          {/* Customer */}
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <h3 className="font-semibold text-gray-700 mb-3">انتخاب مشتری</h3>
            <select value={selectedCustomer} onChange={e => setSelectedCustomer(e.target.value)}
              className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500">
              <option value="">مشتری ناشناس</option>
              {customers.map(c => (
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
            ...
            <div>
              <label className="text-xs text-gray-500">تخفیف (افغانی)</label>
            ...
            {remainingAmount > 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
                <p className="text-sm text-amber-700 font-medium">
                  ⚠ نسیه: {formatCurrency(remainingAmount)}
                </p>
              </div>
            )}
